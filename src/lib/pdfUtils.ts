...
export const generateShoppingListPDF = (items: ShoppingItem[]) => {
  ...
  doc.text('🍽️ Stockely - Lista de Compras', 20, 20);
  ...
  doc.text('Stockely - Sistema de Gestão de Estoque para Restaurantes', 20, pageHeight - 20);
...
export const generateConsumptionReportPDF = (consumptionData: ConsumptionItem[]) => {
  ...
  doc.setFontSize(18);
  doc.setFont('helvetica', 'bold');
  doc.text('🍽️ Stockely - Relatório de Consumo', 20, 20);
...
export const generateMovementReportPDF = (movementSummary: MovementSummary) => {
  ...
  doc.setFontSize(18);
  doc.setFont('helvetica', 'bold');
  doc.text('🍽️ Stockely - Relatório de Movimentações', 20, 20);
